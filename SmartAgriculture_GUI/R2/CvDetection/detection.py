...
from R2.CvDetection.config import *
from R2.CvDetection.TargetBucket import TargetBucket
...
class Detector:
    ...
    HSV_DIST = {
        ...
        "orange": (np.array([96, 120, 120]), np.array([115, 255, 255])),
        # "orange": (np.array([10, 120, 120]), np.array([15, 255, 255])),  # v20230710
        "yellow": (np.array([26, 100, 120]), np.array([95, 255, 255])),
        # "yellow": (np.array([28, 100, 150]), np.array([35, 255, 255]))  # v20230710
    }
    ...
    def __detect_apple(self, rgb_data):
        ...
        targets = circle_detect(mask,
                                {"minDist": 15, "param2": 0.5, "minRadius": 10, "maxRadius": 50})
    ...
    def __detect_pear(self, rgb_data):
        ...
        targets = circle_detect(mask,
                                {"minDist": 15, "param2": 0.1, "minRadius": 15, "maxRadius": 70})
        self.bucket.add_all(targets)
        self.bucket.update()
        ...
        for co, i in enumerate(circles[0, :], start=1):
            ...
            if factor > 0.5:
                if DEBUG:
                    ...
                    cv2.circle(mask, (center_x, center_y), radius, (0, 0, 255), 1)
...
def color_detect(rgb_data, color_lower_bound, color_upper_bound):
    ...
    cnts, hierarchy = cv2.findContours(color_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
