...
from R2.CvDetection.VideoStreamPipe import VideoStreamPipe
from R2.CvDetection.detection import color_detect, circle_detect, Detector
from R2.CvDetection.utli import *
...
class VideoCaptureThread(threading.Thread):
        ...
        if infos_len >= 1:
            if self.get_detect_type() == Detector.FetchType.FETCH.value and infos_len == 1:
                ...
                if self.get_detect_type() == Detector.FetchType.FETCH_ALL.value:
                    ...
                    for fruit in fruit_info_list:
                        if fruit is not None:
                            ...
                            x, y = fruit["x"], fruit["y"]
                            z = depth_frame[int((y - 40) / 2), int((x - 40) / 2)]
                            if DEBUG == True:
                                print("pixel coord X, Y, Z: ", (x, y, z))
                ...
                if len(self.old_real_coord_list) == coord_index:
                    ...
                    # 根据y, x的顺序从左到右排序真实的世界坐标值列表
                    self.real_coord_list = sorted(self.real_coord_list,
                                                  key=lambda x: (x[1], x[0]))  # 0 -> x, 1 -> y, 2 -> z
    ...
    def render_screen(self, rgb_frame, ds_frame):
        rgb_show = cv2.flip(rgb_frame, 1)
        self.rgb_show = cv2.cvtColor(rgb_show, cv2.COLOR_BGR2RGB)
        self.depth_show = ds_frame
        # cv2.imshow("ColorViewer2", rgb_show)
        # cv2.imshow("DepthViewer2", ds_frame)
    ...
    def frame_processing(self, frame_list):
        ...
        if len(frame_list) == 3:
            ...
            self.render_screen(color_frame, depth_show_frame)
    ...
    def run(self):
        while True:
            if DEBUG == True:
                ...
                pass
            frame_list = self.vp.get_color_frame()
            # cv2.namedWindow("ColorViewer2", cv2.WINDOW_AUTOSIZE)
            # cv2.namedWindow("DepthViewer2", cv2.WINDOW_AUTOSIZE)
