from R2.R2Control.MechArmController import MechArmController
...
class RobotR2(MechArmController):
    ...
    def robot_check(self):
        self.status_check()
        self.move_start(default_speed, 2)
    ...
    def __init__(
        ...
        # self.camera_pos = np.array([130, -14, 376])  # 相机到目标的固定坐标点
        self.camera_pos = np.array([130, -5, 376])  # 相机到目标的固定坐标点
    ...
    def pickup_attitude_adjustment_action(self, speed, delay):
        self.ma.send_angles(self.robot_precatch_pose, 80)
        time.sleep(delay)
    ...
    def waiting_point_action(self, waiting_coords, x_pattern, x, y_pattern, y, z_pattern, z, speed):
        ...
        with open('logger.txt', 'a') as f:
            f.write("111" + str(new_coords) + '\n')
        self.ma.send_coords(new_coords, speed)
        time.sleep(2)
        is_position = self.ma.is_in_position(new_coords)
        with open('logger.txt', 'a') as f:
            f.write("222" + str(new_coords) + '\n')
        if is_position == '0':
            with open('logger.txt', 'a') as f:
                f.write("333" + str(new_coords) + str(is_position) + '\n')
        else:
            with open('logger.txt', 'a') as f:
                f.write("444" + str(new_coords) + str(is_position) + '\n')
        # self.set_gripper_range(50, 100)
        # time.sleep(2)
