from R1.R1Control.MechArmController import MechArmController
...
class RobotR1(MechArmController):
    ...
    def __init__(
        ...
        self.conveyor_placement_point = [-88.76, 18.83, -33.57, -0.08, 77.69, 0.52]  # 传送带放置点
        ...
        # self.camera_pos = np.array([119.0, -93.1, 164.9])  # 相机相对于机械臂的固定坐标
        self.camera_pos = np.array([105.0, -94.1, 186.2])  # 相机相对于机械臂的固定坐标
    ...
    def waiting_point_action(self, waiting_coords, x_pattern, x, y_pattern, y, z_pattern, z, speed):
        try:
            ...
            self.ma.send_coords(new_coords, speed, 1)
            time.sleep(2)
    ...
    def pickup_point_action(self, entry_coords, x_pattern, x, y_pattern, y, z_pattern, z, speed):
        try:
            new_coords = self.spatial_adjustment(entry_coords, x_pattern, x, y_pattern, y, z_pattern, z)
            self.ma.send_coords(new_coords, speed, 1)
            time.sleep(2)
            ...
            # gripper_value = self.ma.get_gripper_value()
            # time.sleep(1)
            # while gripper_value > 10:
            #     self.set_gripper_range(0, 100)
            #     time.sleep(2)
            #     break

            # 摘取完果子临时插补点，防止撞到没摘的果子
            self.ma.send_coords(self.spatial_adjustment(self.reacquire_get_coords(), x_pattern, 0.0, '-', 50, '+', 10),
                                speed)
            time.sleep(2.5)
        except Exception as e:
            e = traceback.format_exc()
        ...
        try:
            ...
            if DEBUG:
                with open("log.txt", "a") as f:
                    f.write("self.client.get_current_extracted_count(): " + str(
                        self.client.get_current_extracted_count()) + "\n")
